    ...
    for entry in data:
        ...
        if log_format == 'WSC':
            ...
            if "../" in request or ";" in request:
                vulnerabilities.append(f"[WSC] {ip} - Possible Directory Traversal or Command Injection")
        elif log_format == 'NCA':
...
# Widgets
tk.Label(root, text="Fichier de Log :").grid(row=0, column=0, padx=5, pady=5, sticky="e")
...
tk.Label(root, text="Vulnérabilités Détectées :").grid(row=3, column=0, columnspan=3)
